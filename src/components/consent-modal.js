...
export default class ConsentModal extends React.Component {
    ...
    render() {
        ...
        if (!config.hideDeclineAll && !manager.confirmed)
            declineButton = <button disabled={confirming} className="cm-btn cm-btn-decline cm-btn-right cm-btn-danger" type="button" onClick={declineAndHide}>{t(['decline'])}</button>;
        let acceptAllButton;
        const acceptButton =
